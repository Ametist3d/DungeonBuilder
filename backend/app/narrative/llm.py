...
- content has 1-3 story items only.
- content.type: loot, enemy, trap, npc, hazard.
- Never create keys, scrolls, mechanisms, seals, puzzle tools, secret items, clue items, or ritualObject items.
...
Room ids exactly: [{room_ids}]
Use 1 story content item per room, plus every required unlock item from l.
...
#     if not _lock_rows(context):
#         return "- No required lock keys."

#     return (
#         "- Compact graph field l contains verified door/key placements.\n"
#         "- l row format: [doorId,doorRoom,otherRoom,keyRoom,keyName,keyType,material,lock,gate].\n"
#         "- You must place every keyName in its exact keyRoom.\n"
#         "- Use exact keyType for that content item.\n"
#         "- magicSealed doors use clue content: a scroll.\n"
#         "- puzzleSealed doors use ritualObject content: a mechanism.\n"
#         "- all other closed doors use secret content: a corresponding key.\n"
#         "- Do not move keys, rename keys, merge keys, or place spare keys."
#     )
...
def _is_generated_unlock_duplicate(item: NarrativeContent, required_names: set[str], required_door_ids: set[str]) -> bool:
    ...
    if re.search(r"\b(unlock|opens?|seals?|sealed|mechanism|key|scroll)\b.*\bdoor\b", text):
        return True
...
def _ensure_lock_content(result: DungeonNarrative, context: dict) -> None:
    ...
    for row in _lock_rows(context):
        door_id, door_room, other_room, key_room, key_name, key_type, material, lock, gate = row
