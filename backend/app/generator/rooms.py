...
def _used_directions(rooms: list[RoomNode], corridors: list[Corridor]) -> dict[int, set[str]]:
...
def _pick_open_wall(used: set[str], rng: random.Random) -> str:
...
def generate_dungeon(
    ...
    entrance_dir = _pick_open_wall(used[root_room.id], rng)
    used[root_room.id].add(entrance_dir)  # so a single-room dungeon doesn't reuse the same wall
    exit_dir = _pick_open_wall(used[last_room.id], rng)
