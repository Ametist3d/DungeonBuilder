...
function createHeroSvg(): SVGGElement {
  ...
  hero.innerHTML = `
    ...
    <path class="hero-cloak" d="M0,-10 C6,-8 9,-2 8,7 C4,10 -4,10 -8,7 C-9,-2 -6,-8 0,-10 Z"/>
    <circle class="hero-face" cx="0" cy="-3" r="4"/>
    <path class="hero-hood" d="M-6,-3 C-4,-10 4,-10 6,-3 C3,-6 -3,-6 -6,-3 Z"/>
    <path class="hero-blade" d="M6,-7 L11,-12 M9,-13 L12,-10"/>
  `;
...
function pickupAtCurrentCell(): void {
  ...
  if (!marker || marker.element?.classList.contains('picked')) return;

  const tokens = pickupTokens(marker);
  if (!tokens.length) return;

  tokens.forEach((token) => heroState.keys.add(token));
