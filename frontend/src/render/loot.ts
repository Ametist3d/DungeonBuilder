...
function moveTooltip(event: PointerEvent): void {
  ...
  if (rect.right > window.innerWidth - 8) {
    tooltip.style.left =
      `${event.clientX - rect.width - gap}px`;
  }

  if (rect.bottom > window.innerHeight - 8) {
    tooltip.style.top =
      `${event.clientY - rect.height - gap}px`;
  }
}
